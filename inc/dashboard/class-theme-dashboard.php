...
class Base_Dashboard_Settings {
	...
	public function add_menu() {
		$page = add_theme_page( __( 'Rebase - Next Generation Theme', 'rebase' ), __( 'Rebase', 'rebase' ), apply_filters( 'base_admin_settings_capability', 'manage_options' ), 'rebase', array( $this, 'config_page' ) );
	...
	public function scripts() {
		...
		$button_label = esc_html__( 'Browse Base Starter Templates', 'rebase' );
		$data_action  = '';
		...
		wp_enqueue_style( 'base-dashboard', get_template_directory_uri() . '/inc/dashboard/react/dash-controls.min.css', array( 'wp-components' ), REBASE_VERSION );
		wp_enqueue_script( 'base-dashboard', get_template_directory_uri() . '/assets/js/admin/dashboard.js', array( 'wp-i18n', 'wp-element', 'wp-plugins', 'wp-components', 'wp-api', 'wp-hooks', 'wp-edit-post', 'lodash', 'wp-block-library', 'wp-block-editor', 'wp-editor', 'jquery' ), REBASE_VERSION, true );
		wp_localize_script(
			'base-dashboard',
			'baseDashboardParams',
			array(
				'adminURL' => esc_url( admin_url() ),
				'settings' => esc_attr( get_option( 'rebase_theme_config' ) ),
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'base-dashboard', 'rebase' );
		}
	}
	/**
	...
	public function load_api_settings() {
		...
		register_setting(
			...
			array(
				'type'              => 'string',
				'description'       => __( 'Config Theme Modules', 'rebase' ),
			...
			<div class="base_theme_dash_head">
				<div class="base_theme_dash_head_container">
					<div class="base_theme_dash_logo">
						<img src="<?php echo esc_attr( get_template_directory_uri() . '/assets/images/rebase-logo.png' ); ?>">
					</div>
					<div class="base_theme_dash_version">
						<span>
							<?php echo esc_html( REBASE_VERSION ); ?>
						</span>
					</div>
				</div>
			</div>
					...
					<div class="side-panel">
						<div class="support-section sidebar-section components-panel">
							<div class="components-panel__body is-opened">
								<h2><?php esc_html_e( 'Video Tutorials', 'rebase' ); ?></h2>
								<p><?php esc_html_e( 'Want a guide? We have video tutorials to walk you through getting started.', 'rebase' ); ?></p>
								<a href="https://github.com/wpdesignx/rebase#learn" target="_blank" class="sidebar-link"><?php esc_html_e( 'Watch Videos', 'rebase' ); ?></a>
							</div>
						</div>
						<div class="support-section sidebar-section components-panel">
							<div class="components-panel__body is-opened">
								<h2><?php esc_html_e( 'Documentation', 'rebase' ); ?></h2>
								<p><?php esc_html_e( 'Need help? We have a knowledge base full of articles to get you started.', 'rebase' ); ?></p>
								<a href="https://github.com/wpdesignx/rebase#kb" target="_blank" class="sidebar-link"><?php esc_html_e( 'Browse Docs', 'rebase' ); ?></a>
							</div>
						</div>
						<div class="support-section sidebar-section components-panel">
							<div class="components-panel__body is-opened">
								<h2><?php esc_html_e( 'Support', 'rebase' ); ?></h2>
								<p><?php esc_html_e( 'Have a question, we are happy to help! Get in touch with our support team.', 'rebase' ); ?></p>
								<a href="https://github.com/wpdesignx/rebase#support" target="_blank" class="sidebar-link"><?php esc_html_e( 'Submit a Ticket', 'rebase' ); ?></a>
							</div>
						</div>
					</div>
				</div>
