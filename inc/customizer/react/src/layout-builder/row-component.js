...
class RowComponent extends Component {
	render() {
		...
		let besideItems = [];
		return (
			<div className={ `base-builder-areas ${ centerClass }` }>
				<Button
					className="base-row-left-actions"
					aria-label={ __( 'Edit Settings for', 'rebase' ) + ' ' + ( this.props.row === 'popup' ? __( 'Off Canvas', 'rebase' ) : this.props.row + ' ' + __( 'Row', 'rebase' ) ) }
					onClick={ () => this.props.focusPanel( mode + '_' + this.props.row ) }
					icon="admin-generic"
				>
				</Button>
				<Button
					className="base-row-actions"
					aria-label={ __( 'Edit Settings for', 'rebase' ) + ' ' + ( this.props.row === 'popup' ? __( 'Off Canvas', 'rebase' ) : this.props.row + ' ' + __( 'Row', 'rebase' ) ) }
					onClick={ () => this.props.focusPanel( mode + '_' + this.props.row ) }
				>
					{ ( this.props.row === 'popup' ? __( 'Off Canvas', 'rebase' ) : this.props.row + ' ' + __( 'Row', 'rebase' ) ) }
					<Dashicon icon="admin-generic" />
				</Button>
				<div className="base-builder-group base-builder-group-horizontal" data-setting={ this.props.row }>
