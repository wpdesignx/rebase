	...
	render() {
		...
		return (
			<div className={'base-font-pair-wrap'}>
				<Button className={ 'base-font-pair-btn' } label={ __( 'Choose a font', 'rebase' ) } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
					<Dashicon icon="portfolio" />
				</Button>
				{ this.state.isVisible && (
					<Popover position="bottom left" className="base-font-pair-popover" onClose={ toggleClose }>
						<h2 style={{ textAlign:'center' }}>{ __( 'Select a Font Pairing', 'rebase' ) }</h2>
						<ButtonGroup className="bst-font-pair-group" aria-label={ __( 'Select a Font Pair', 'rebase' ) }>
							{ map( this.state.fonts, ( { hfont, bfont, hv, img, name } ) => {
								return (
									this.state.confirm === name ? (
										...
										>
											{ __( 'Confirm Change Settings?', 'rebase' ) }
										</Button>
									) : (
										<Button
