...
import BaseGradientPicker from './gradient-component.js';
import BaseColorPicker from '../common/color-picker';
...
class BackgroundComponent extends Component {
	constructor(props) {
		...
		let defaultParams = {
			...
			repeat: {
				'no-repeat': {
					name: __( 'No Repeat', 'rebase' ),
				},
				'repeat': {
					name: __( 'Repeat', 'rebase' ),
				},
				'repeat-x': {
					name: __( 'Repeat-X', 'rebase' ),
				},
				'repeat-y': {
					name: __( 'Repeat-y', 'rebase' ),
				},
			},
			size: {
				auto: {
				...
				cover: {
					name: __( 'Cover', 'rebase' ),
				},
				contain: {
					name: __( 'Contain', 'rebase' ),
				},
			},
			attachment: {
				scroll: {
					name: __( 'Scroll', 'rebase' ),
				},
				fixed: {
					name: __( 'Fixed', 'rebase' ),
				},
			},
			attachments: {
				desktop: {},
		...
		const palette = JSON.parse( this.props.customizer.control( 'base_color_palette' ).setting.get() );
		//console.log( palette );
		this.state = {
			value: value,
	...
	render() {
		...
		const controlLabel = (
			<Fragment>
				{ this.state.currentDevice !== 'desktop' && (
					<Tooltip text={ __( 'Reset Device Values', 'rebase' ) }>
						<Button
		...
		if( this.state.supportGradient ) {
			tab_options = [
				{
					name: 'color',
					title: __( 'Color', 'rebase' ),
					className: 'base-color-background',
				},
				{
					name: 'gradient',
					title: __( 'Gradient', 'rebase' ),
					className: 'base-gradient-background',
				},
				{
					name: 'image',
					title: __( 'Image', 'rebase' ),
					className: 'base-image-background',
				},
			];
		} else {
			tab_options = [
		...
		return (
			<div className="base-control-field base-background-control">
				<ResponsiveControl
				...
				>
					<div className="base-background-picker-wrap">
						{ this.state.isVisible && (
							<Popover position="top left" className="base-popover-color" onClose={ maybeToggleClose }>
												...
												if ( 'image' === tab.name ) {
													tabout = (
														<Fragment>
															...
															<span className="customize-control-title">{ __( 'Background Repeat', 'rebase' ) }</span>
															...
															<span className="customize-control-title">{ __( 'Background Size', 'rebase' ) }</span>
															<ButtonGroup className="base-radio-container-control">
															...
															<span className="customize-control-title">{ __( 'Background Attachment', 'rebase' ) }</span>
															<ButtonGroup className="base-radio-container-control">
						...
						<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Background', 'rebase' ) }>
							<div className="background-button-wrap">
								<Button className={ 'base-background-icon-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
									{ ( undefined === this.state.value[ this.state.currentDevice ] || undefined === this.state.value[ this.state.currentDevice ].type || this.state.value[ this.state.currentDevice ].type === 'color' ) && (
										<Fragment>
											<ColorIndicator className="base-advanced-color-indicate" colorValue={ getColorPreviewValue( this.state.currentDevice ) } />
											...
											{ showGradientPreview( this.state.currentDevice ) &&
												<ColorIndicator className="base-advanced-color-indicate" colorValue={ getGradientPreviewValue( this.state.currentDevice ) } />
											}
										</Fragment>
									) }
	...
	preventClose() {
		let self = this;
		document.addEventListener( 'baseOpenMediaModal', function(e) {
