...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function base_welcome_notice() {
		if ( ! defined( 'REBASE_VERSION' ) || get_option( 'base_disable_welcome_notice' ) ) {
			return;
		}
		?>
		<div id="base-notice-welcome-plugin" class="notice is-dismissible notice-info">
			<h2 class="notice-title"><?php echo esc_html__( 'Thanks for choosing the Rebase Theme!', 'rebase' ); ?></h2>
			<p class="base-notice-description"><?php echo esc_html__( 'You can customize the styling of your theme - including header fooder layout, font, colors and spacing - in your theme settings.', 'rebase' ); ?></p>
			<?php if ( !isset( $_GET['page'] ) || $_GET['page'] !== 'rebase' ) { ?>
			<p class="install-submit">
				<a href="<?php echo esc_url( admin_url( 'themes.php?page=rebase' ) ); ?>"><button class="button button-primary base-theme-settings"><?php echo esc_html__( 'Go to Theme Settings', 'rebase' ); ?></button></a>
			</p>
			<?php } ?>
		</div>
		<?php
	...
	public function base_starter_templates_notice() {
		...
		if ( ! isset( $installed_plugins['base-starter/base-starter.php'] ) ) {
			$button_label = esc_html__( 'Install Base Starter Templates', 'rebase' );
			$data_action  = 'install';
		} elseif ( ! self::active_plugin_check( 'base-starter/base-starter.php' ) ) {
			$button_label = esc_html__( 'Activate Base Starter Templates', 'rebase' );
		...
		<div id="base-notice-starter-templates" class="notice is-dismissible notice-info">
			<h2 class="notice-title"><?php echo esc_html__( 'Thanks for choosing the Rebase Theme!', 'rebase' ); ?></h2>
			<p class="base-notice-description"><?php /* translators: %s: <strong> <a> */ printf( esc_html__( 'Want to get started with a beautiful %1$sstarter template%2$s? Install the Base Starter Templates plugin to launch an optimized design in minutes.', 'rebase' ), '<a href="https://wordpress.org/plugins/base-starter-templates/" target="_blank">', '</a>', '<strong>', '</strong>' ); ?></p>
			<p class="install-submit">
				<button class="button button-primary base-install-starter-btn" data-redirect-url="<?php echo esc_url( admin_url( 'themes.php?page=base-starter' ) ); ?>" data-activating-label="<?php echo esc_attr__( 'Activating...', 'rebase' ); ?>" data-activated-label="<?php echo esc_attr__( 'Activated', 'rebase' ); ?>" data-installing-label="<?php echo esc_attr__( 'Installing...', 'rebase' ); ?>" data-installed-label="<?php echo esc_attr__( 'Installed', 'rebase' ); ?>" data-action="<?php echo esc_attr( $data_action ); ?>"><?php echo esc_html( $button_label ); ?></button>
			</p>
		</div>
		<?php
	...
	public function base_turn_off_gutenberg_plugin_notice() {
		...
		<div id="base-notice-gutenberg-plugin" class="notice is-dismissible notice-warning">
			<h2 class="notice-title"><?php echo esc_html__( 'Gutenberg Plugin Detected', 'rebase' ); ?></h2>
			<p class="base-notice-description"><?php /* translators: %s: <a> */ printf( esc_html__( 'The %1$sGutenberg plugin%2$s is not recommended for use in a production site. Many things may be broken by using this plugin. Please deactivate.', 'rebase' ), '<a href="https://wordpress.org/plugins/gutenberg/" target="_blank">', '</a>' ); ?></p>
		</div>
		<?php
	...
	public function install_starter_script() {
		wp_register_script( 'base-welcome-deactivate', get_template_directory_uri() . '/assets/js/welcome-notice.min.js', array( 'jquery' ), REBASE_VERSION, false );
		wp_register_script( 'base-starter-install', get_template_directory_uri() . '/assets/js/admin-activate.min.js', array( 'jquery' ), REBASE_VERSION, false );
		wp_register_script( 'base-gutenberg-deactivate', get_template_directory_uri() . '/assets/js/gutenberg-notice.min.js', array( 'jquery' ), REBASE_VERSION, false );
	}
	/**
	 * Active Plugin Check
	 *
	...
	public function install_plugin_ajax_callback() {
		if ( ! check_ajax_referer( 'base-ajax-verification', 'security', false ) ) {
			wp_send_json_error( __( 'Security Error, Please reload the page.', 'rebase' ) );
		}
		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( __( 'Security Error, Need higher Permissions to install plugin.', 'rebase' ) );
		}
		...
		if ( false === $install ) {
			wp_send_json_error( __( 'Error, plugin could not be installed, please install manually.', 'rebase' ) );
		} else {
	...
	public static function customizer_quick_link() {
		if ( is_customize_preview() ) {
			...
			<div class="customize-partial-edit-shortcut base-custom-partial-edit-shortcut">
				<button aria-label="<?php esc_attr_e( 'Click to edit this element.', 'rebase' ); ?>" title="<?php esc_attr_e( 'Click to edit this element.', 'rebase' ); ?>" class="customize-partial-edit-shortcut-button item-customizer-focus"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M13.89 3.39l2.71 2.72c.46.46.42 1.24.03 1.64l-8.01 8.02-5.56 1.16 1.16-5.58s7.6-7.63 7.99-8.03c.39-.39 1.22-.39 1.68.07zm-2.73 2.79l-5.59 5.61 1.11 1.11 5.54-5.65zm-2.97 8.23l5.58-5.6-1.07-1.08-5.59 5.6z"></path></svg></button>
			</div>
			<?php
		}
	}
