<?php
/**
 * Base\Comments\Component class
 *
 * @package Base
 */

namespace Base\Comments;
 ...
 * * `webapp()->the_comments( array $args = [] )`
 *
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function the_comments( array $args = array() ) {
		...
		$amp_live_list = webapp()->using_amp_live_list_comments();
		...
		if ( $amp_live_list ) {
			...
			?>
				<div update>
					<button class="button" on="tap:amp-live-comments-list-<?php the_ID(); ?>.update"><?php esc_html_e( 'New comment(s)', 'rebase' ); ?></button>
				</div>
			</amp-live-list>
			<?php
		}
	...
	public function filter_default_fields_markup( $fields ) {
		...
		$show_web  = webapp()->option( 'comment_form_remove_web' );
		$fields['author'] = '<div class="comment-input-wrap ' . ( $show_web ? 'no-url-field' : 'has-url-field' ) . '"><p class="comment-form-author"><input aria-label="' . esc_attr__( 'Name', 'rebase' ) . '" id="author" name="author" type="text" placeholder="' . esc_attr__( 'John Doe', 'rebase' ) . '" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" maxlength="245"' . $aria_req . ' /><label class="float-label" for="author">' . esc_html__( 'Name', 'rebase' ) . $label_req . '</label></p>';

		$fields['email'] = '<p class="comment-form-email"><input aria-label="' . esc_attr__( 'Email', 'rebase' ) . '" id="email" name="email" type="email" placeholder="' . esc_attr__( 'john@example.com', 'rebase' ) . '" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" maxlength="100" aria-describedby="email-notes"' . $aria_req . ' /><label class="float-label" for="email">' . esc_html__( 'Email', 'rebase' ) . $label_req . '</label></p>';
		if ( $show_web ) {
			$fields['url'] = '</div>';
		} else {
			$fields['url'] = '<p class="comment-form-url"><input aria-label="' . esc_attr__( 'Website', 'rebase' ) . '" id="url" name="url" type="url" placeholder="' . esc_attr__( 'https://www.example.com', 'rebase' ) . '" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" maxlength="200" /><label class="float-label" for="url">' . esc_html__( 'Website', 'rebase' ) . '</label></p></div>';
		}
