 ...
 *
 * Its instance can be accessed through `webapp()`. For example, if there is a template tag called `posted_on`, it can
			...
			if ( ! $component instanceof Templating_Component_Interface ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The theme templating component %1$s does not implement the %2$s interface.', 'rebase' ),
						gettype( $component ),
		...
		if ( ! isset( $this->template_tags[ $method ] ) ) {
			throw new BadMethodCallException(
				sprintf(
					...
					__( 'The template tag %s does not exist.', 'rebase' ),
					'webapp()->' . $method . '()'
				)
			);
		}
		...
		foreach ( $tags as $method_name => $callback ) {
			...
			if ( ! is_array( $callback ) || ! isset( $callback['callback'] ) ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The template tag method %1$s registered by theme component %2$s must either be a callable or an array.', 'rebase' ),
						$method_name,
						get_class( $component )
					)
				);
			...
			if ( isset( $this->template_tags[ $method_name ] ) ) {
				throw new RuntimeException(
					sprintf(
						/* translators: 1: template tag method name, 2: component class name */
						__( 'The template tag method %1$s registered by theme component %2$s conflicts with an already registered template tag of the same name.', 'rebase' ),
						$method_name,
						get_class( $component )
					)
				);
