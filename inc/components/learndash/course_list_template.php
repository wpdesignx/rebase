<?php
/**
 * This template overrides the learndash grid output for better support with the Theme.
 *
 * @package Base
 */
...
$boxed = Base\webapp()->option( 'learndash_course_grid_style' );
...
$button_text = isset( $button_text ) && ! empty( $button_text ) ? $button_text : __( 'See more...', 'rebase' );
...
if ( function_exists( 'learndash_get_course_price' ) ) {
	...
	$price = $course_options && isset( $course_options['sfwd-courses_course_price'] ) ? $course_options['sfwd-courses_course_price'] : __( 'Free', 'rebase' );
