...
function footer_html() {
	...
	if ( $content || is_customize_preview() ) {
		...
		// translators: %s is link to Rebase".
		$content = str_replace( '{theme-credit}', sprintf( __( '- WordPress Theme by %s', 'rebase' ), '<a href="https://github.com/wpdesignx" rel="nofollow noopener" target="_blank">Rebase</a>' ), $content );
...
function footer_navigation() {
	?>
	<nav id="footer-navigation" class="footer-navigation" role="navigation" aria-label="<?php esc_attr_e( 'Footer Navigation', 'rebase' ); ?>">
...
function scroll_up() {
	if ( webapp()->option( 'scroll_up' ) ) {
		echo '<a id="bst-scroll-up" tabindex="-1" aria-hidden="true" aria-label="' . esc_attr__( 'Scroll to top', 'rebase' ) . '" href="#wrapper" class="base-scroll-to-top scroll-up-wrap scroll-ignore scroll-up-side-' . esc_attr( webapp()->option( 'scroll_up_side' ) ) . ' scroll-up-style-' . esc_attr( webapp()->option( 'scroll_up_style' ) ) . ' vs-lg-' . ( webapp()->sub_option( 'scroll_up_visiblity', 'desktop' ) ? 'true' : 'false' ) . ' vs-md-' . ( webapp()->sub_option( 'scroll_up_visiblity', 'tablet' ) ? 'true' : 'false' ) . ' vs-sm-' . ( webapp()->sub_option( 'scroll_up_visiblity', 'mobile' ) ? 'true' : 'false' ) . '">';
		webapp()->print_icon( webapp()->option( 'scroll_up_icon' ), esc_attr__( 'Scroll to top', 'rebase' ), false );
		echo '</a>';
		echo '<button id="bst-scroll-up-reader" href="#wrapper" aria-label="' . esc_attr__( 'Scroll to top', 'rebase' ) . '" class="base-scroll-to-top scroll-up-wrap scroll-ignore scroll-up-side-' . esc_attr( webapp()->option( 'scroll_up_side' ) ) . ' scroll-up-style-' . esc_attr( webapp()->option( 'scroll_up_style' ) ) . ' vs-lg-' . ( webapp()->sub_option( 'scroll_up_visiblity', 'desktop' ) ? 'true' : 'false' ) . ' vs-md-' . ( webapp()->sub_option( 'scroll_up_visiblity', 'tablet' ) ? 'true' : 'false' ) . ' vs-sm-' . ( webapp()->sub_option( 'scroll_up_visiblity', 'mobile' ) ? 'true' : 'false' ) . '">';
		webapp()->print_icon( webapp()->option( 'scroll_up_icon' ), esc_attr__( 'Scroll to top', 'rebase' ), false );
		echo '</button>';
	}
}
