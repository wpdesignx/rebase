...
<article <?php post_class( 'entry content-bg loop-entry' ); ?>>
	<?php
		...
		$feature_element = webapp()->option( $slug . '_archive_element_feature', $defaults );
...
if ( is_singular( get_post_type() ) ) {
	...
	if ( ! empty( $post->post_parent ) ) {
		...
		if ( ! empty( $prev_link ) ) {
			...
			$attachment_navigation .= '<div class="post-navigation-sub"><span>' . esc_html__( 'Previous:', 'rebase' ) . '</span></div>';
		...
		if ( ! empty( $next_link ) ) {
			...
			$attachment_navigation .= '<div class="post-navigation-sub"><span>' . esc_html__( 'Next:', 'rebase' ) . '</span></div>';
			$attachment_navigation .= $next_link;
			$attachment_navigation .= '</div>';
		}

		if ( ! empty( $attachment_navigation ) ) {
			echo _navigation_markup( $attachment_navigation, $class = 'post-navigation', __( 'Post navigation', 'rebase' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
