...
if ( have_comments() ) {
	...
	if ( 1 === $comment_count ) {
		$title_output .= esc_html__( 'One Comment', 'rebase' );
	} else {
		$title_output .= sprintf(
			/* translators: 1: comment count number */
			esc_html( _nx( '%1$s Comment', '%1$s Comments', $comment_count, 'comments title', 'rebase' ) ),
	...
	if ( ! comments_open() ) {
		?>
		<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'rebase' ); ?></p>
		<?php
	}
