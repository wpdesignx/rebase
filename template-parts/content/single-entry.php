	...
	if ( ( 'post' === get_post_type() || get_post_type_object( get_post_type() )->has_archive ) && webapp()->show_post_navigation() ) {
		...
		the_post_navigation(
			apply_filters(
				...
				array(
					'prev_text' => '<div class="post-navigation-sub"><small>' . webapp()->get_icon( 'arrow-left-alt' ) . esc_html__( 'Previous', 'rebase' ) . '</small></div>%title',
					'next_text' => '<div class="post-navigation-sub"><small>' . esc_html__( 'Next', 'rebase' ) . webapp()->get_icon( 'arrow-right-alt' ) . '</small></div>%title',
				)
			)
		);
		if ( webapp()->option( 'post_footer_area_boxed' ) ) {
